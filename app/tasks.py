...
def create_asset_async(username,serial_no,cost,priv_key,no):
    try:
        ...
        user_obj.launch_task("create_n_assets",f"CREATE task queued for {no} assets",username,serial_no,cost,priv_key,no)
        return True
    except:
...
def transfer_asset_async(username,serial_no,priv_key,no,assets):
    try:
        ...
        while no > ASSETS_PER_TRANS:
            ...
            username,serial_no,priv_key,ASSETS_PER_TRANS,assets[counter:counter+ASSETS_PER_TRANS])
            no -= ASSETS_PER_TRANS
            counter += ASSETS_PER_TRANS
        temp = -1*no
        user_obj.launch_task("transfer_n_assets",f"TRANSFER task queued for {ASSETS_PER_TRANS} assets",
            username,serial_no,priv_key,no,assets[temp:])
        return True
    except:
        app.logger.error('Unhandled exception', exc_info=sys.exc_info())
        return False
